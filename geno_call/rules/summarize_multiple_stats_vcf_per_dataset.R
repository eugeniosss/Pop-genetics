...
read_file <- function(directory, filename, software) {
...
software_names <- gsub("/stats/", "", input_dirs)

# Read and merge data from all software directories
for (i in seq_along(input_dirs)) {

    dir <- input_dirs[i]
    software <- software_names[i]

    all_data$var_qual <- bind_rows(all_data$var_qual, read_file(dir, "site_quality.lqual", software))
    all_data$var_depth <- bind_rows(all_data$var_depth, read_file(dir, "mean_depth_per_site.ldepth.mean", software))
    all_data$var_miss <- bind_rows(all_data$var_miss, read_file(dir, "missing_data_per_site.lmiss", software))
    all_data$var_freq <- bind_rows(all_data$var_freq, read_file(dir, "allele_freq2.frq", software))
    all_data$ind_depth <- bind_rows(all_data$ind_depth, read_file(dir, "mean_depth_per_individual.idepth", software))
    all_data$ind_miss <- bind_rows(all_data$ind_miss, read_file(dir, "missing_data_per_indv.imiss", software))
}
...
plot_list <- list(
    ggplot(all_data$var_qual %>% filter(!is.na(X3)), aes(x = X3, color = Software)) +
        stat_density(aes(x=X3, colour=Software), geom="line",position="identity") +
        labs(title = "Variant Quality Distribution", x = "Quality Score", y = "Density") +
        theme_light(),

    ggplot(all_data$var_depth %>% filter(!is.na(X3)), aes(x = X3, color = Software)) +
        stat_density(aes(x=X3, colour=Software), geom="line",position="identity") +
        labs(title = "Variant Depth Distribution", x = "Mean Depth", y = "Density") +
        theme_light(),

    ggplot(all_data$var_miss %>% filter(!is.na(X6)), aes(x = X6, color = Software)) +
        stat_density(aes(x=X6, colour=Software), geom="line",position="identity") +
        labs(title = "Variant Missingness Distribution", x = "Proportion of Missing Data", y = "Density") +
        theme_light(),

    ggplot(all_data$var_freq %>% mutate(MAF = pmin(X5, X6)) %>% filter(!is.na(MAF)), aes(x = MAF, color = Software)) +
        stat_density(aes(x=MAF, colour=Software), geom="line",position="identity") +
        labs(title = "Minor Allele Frequency Distribution", x = "Minor Allele Frequency", y = "Density") +
        theme_light(),

    ggplot(all_data$ind_depth %>% filter(!is.na(X3)), aes(x = X3, fill = Software)) +
    ...
    ggplot(all_data$ind_miss %>% filter(!is.na(X5)), aes(x = X5, fill = Software)) +
        geom_histogram(position = "identity", alpha = 0.25, bins = 30, na.rm = TRUE) +
...
missing_data_table <- all_data$ind_miss %>%
    ...
    select(ind, X5, Software) %>%
    pivot_wider(
        names_from = Software,      # Make the software names the column names
...
snps_df_cleaned <- data.frame(
  Software = names(total_snps_cleaned),
  Total_SNPs = total_snps_cleaned
)

ggplot(snps_df_cleaned, aes(x = Software, y = Total_SNPs, fill = Software)) +
